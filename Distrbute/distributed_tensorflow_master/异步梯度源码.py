      ...
      for grad, var in grads_and_vars:
        ...
        with ops.device(var.device):
          ...
          if grad is None:
            ...
            '''SparseConditionalAccumulator稀疏渐变由IndexedSlices表示。
...
平均渐变的提取被阻止，直到所需数量的渐变被累积。'''
          ...
          self._accumulator_list.append((grad_accum, var.device))      #梯度和变量设备列表（累加器）
...
#的点点滴滴多
      ...
      with ops.device(global_step.device), ops.name_scope(""):
        ...
        with ops.control_dependencies([update_op]):
          ...
          tokens = array_ops.fill([self._tokens_per_step], global_step)     #最大worker数量，现在换成step
  ...
  def get_chief_queue_runner(self):
    ...
    """
    '''
    msgstr“”“返回要执行的主管的QueueRunner。

     这包括同步副本的操作：聚合渐变，
     应用于变量，增加全局步骤，将令牌插入令牌队列。

     请注意，只有在调用apply_gradients（）之后才能调用它
     实际上生成这个queuerunner。
...
    举：
       ValueError：如果在apply_gradients（）之前调用它。
    '''
