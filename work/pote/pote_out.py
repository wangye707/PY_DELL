...
def neural_network(model = 'lstm', rnn_size = 128, num_layers = 2):
    ...
    with g.as_default():
        ...
        softmax_w = tf.get_variable([rnn_size, len(words)])
        softmax_b = tf.get_variable([len(words)])
        embedding = tf.get_variable([len(words), rnn_size])
...
def gen_head_poetry(heads, type):
    ...
    saver = tf.train.import_meta_graph('./model/' + "poetry.module-124.meta")
